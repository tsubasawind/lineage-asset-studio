...
Catalog Mode 可匯入遊戲資料 ZIP 作為中文名稱參考，並以穩定的 `nameId` 將素材連結到 `.LineageAssetStudio/data/names_catalog.json`。重新命名會保留舊名稱作為搜尋別名；原始流水號與圖片不會被修改。匯出時可選擇流水號、中文名稱＋UID，或中文名稱作為檔名。
